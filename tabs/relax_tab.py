...
def _plot_energy(energies):
    ...
    plt.plot(np.arange(len(energies)), energies, marker="o", linewidth=2)
...
def relax_tab(pmg_obj):
    ...
    stride = st.number_input("Save every Nth step", value=10, min_value=1, max_value=1000, step=1)
    ...
    if not run:
        # Keep download buttons visible if they exist
        if st.session_state.relaxed_cif:
            st.download_button("⬇️ Download relaxed CIF", st.session_state.relaxed_cif, "relaxed_structure.cif")
        if st.session_state.relax_traj_xyz:
            st.download_button("⬇️ Download trajectory (XYZ)", st.session_state.relax_traj_xyz, "relax_trajectory.xyz")
        return
    ...
    try:
        ...
        def on_step():
            ...
            if buf_xyz is not None and (i % int(stride) == 0):
                frame = atoms.copy()
        ...
        st.download_button("⬇️ Download relaxed CIF", st.session_state.relaxed_cif, file_name="relaxed_structure.cif")

        if buf_xyz is not None:
            st.session_state.relax_traj_xyz = buf_xyz.getvalue().encode()
            st.download_button("⬇️ Download trajectory (XYZ)", st.session_state.relax_traj_xyz, file_name="relax_trajectory.xyz")
